...
export const EditRoomInfor = () => {
  ...
  const update = query.get("cap-nhat-thong-tin-phong");
  const { phong, khus } = useContext(PhongContext);
  ...
  useEffect(() => {
    if (update) {
      setFirst(true);
      if (phong.ten_phong === update) {
        formik.setFieldValue("ten_phong", phong.ten_phong);
        formik.setFieldValue("tang", phong.tang);
        formik.setFieldValue("don_gia", phong.don_gia);
        formik.setFieldValue("dien_tich", phong.dien_tich);
        formik.setFieldValue("so_nguoi_phu_hop", phong.so_nguoi_phu_hop);
        formik.setFieldValue("tien_coc", phong.tien_coc);
        formik.setFieldValue("ngay_trong", phong.ngay_trong);
      }
      setTimeout(() => {
        setOpen(true);
      }, 300);
            ...
            <div className="sm:flex sm:items-start">
              <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                <h3
                  ...
                  id="modal-title"
                >
                  Cap nhat thong tin phong {update} -{" "}
                  {khus.filter((el: any) => {
